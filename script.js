...
$(function () {
  ...
  var closeFb = function () {
    ...
    positionFb = function ($activeFb) {
      ...
      var offsetY =
          $(window).scrollTop() -
          $img.offset().top +
          0.5 * ($img.data("imgHeight") * ($img.data("imgScale") - 1)) +
          0.5 *
            ($(window).height() -
              $img.data("imgHeight") * $img.data("imgScale")),
        offsetX =
          0.5 * ($img.data("imgWidth") * ($img.data("imgScale") - 1)) +
          0.5 *
            ($(window).width() -
              $img.data("imgWidth") * $img.data("imgScale")) -
          $img.offset().left,
      ...
      $ghost.css({
        transform:
          "translate(" +
          parseInt(offsetX * 10) / 10 +
          "px," +
          parseInt(offsetY * 10) / 10 +
          "px) scale(" +
          parseInt(scale * 1000) / 1000 +
          ")",
      });
    };
  ...
  $fb.imagesLoaded().done(function () {
    ...
    $fb.click(function (e) {
      ...
      if (
        $(this).data("fluidbox-state") === 0 ||
        !$(this).data("fluidbox-state")
      ) {
        // State: Closed
